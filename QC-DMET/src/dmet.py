...
class dmet:
    def __init__(self,
        ...
        ##?impurity method
        assert ((SCmethod == 'LSTSQ') or (SCmethod == 'BFGS')
                or (SCmethod == 'NONE'))  ##sel-consistmethod

        self.ints = theInts
        self.Norb = self.ints.Norbs  #number of orbital
        ...
        self.print_u = True
        self.print_rdm = True  #rdm 约化密度矩阵

        allOne = self.testclusters()
        if (allOne == False
            ):  # One or more impurities which do not cover the entire system
            assert (self.TransInv == False
                    )  # Make sure that you don't work translational invariant
    ...
    def doexact(self, chempot_imp=0.0):
        ...
        maxiter = len(self.impClust)  #impClust是list，每个元素是1/0表示这个轨道是不是杂杂质？
        if (self.TransInv == True):  # 平移的话只看一个，因为impClust就是平移出来的
            maxiter = 1
        ...
        for counter in range(maxiter):
            ...
            impurityOrbs = np.abs(self.impClust[counter])
            numImpOrbs = np.sum(impurityOrbs)
            if (self.BATH_ORBS == None):  #默认Bath和Imp轨道一样多
            ...
            if (self.BATH_ORBS == None):
                ...
                core_cutoff = 0.5
            for cnt in range(len(core1RDM_dmet)):# 规整一下返回的本征值，这个本征值要么是0要么是2，代表电子个数
            ...
            core1RDM_loc = np.dot(np.dot(loc2dmet, np.diag(core1RDM_dmet)),
                                  loc2dmet.T)#得到了去掉杂质环境和环境中没有占据的态的密度矩阵

            self.dmetOrbs.append(
                loc2dmet[:, :Norb_in_imp])  # Impurity and bath orbitals only，杂质部分是单位阵
            assert (Norb_in_imp <= self.Norb)
            dmetOEI = self.ints.dmet_oei(loc2dmet,
                                         Norb_in_imp)  #hopping在loc2dmet的active空间的投影, 有效哈密顿量
            dmetFOCK = self.ints.dmet_fock(
                loc2dmet, Norb_in_imp,
                core1RDM_loc)  #core1RDM_loc在loc2dmet的非active空间的投影
            dmetTEI = self.ints.dmet_tei(
                loc2dmet, Norb_in_imp
            )  #计算四算符关联函数？dmetTEI是一个numAct*numAct*numAct*numAct的张量，dmetTEI[i,j,k,l] = sum(loc2dmet的第i,j,k,l列位乘)
    ...
    def constructNOrotation(self):
        ...
        for count in range(len(
                self.imp_size)):  # self.imp_size has length 1 if self.TransInv
            myNOrotation[jumpsquare:jumpsquare + self.imp_size[count],
                         jumpsquare:jumpsquare +
                         self.imp_size[count]] = self.NOvecs[count]
        ...
        if (self.TransInv == True):
            ...
            for it in range(1, self.Norb / size):
                myNOrotation[it * size:(it + 1) * size,
                             it * size:(it + 1) * size] = myNOrotation[0:size,
                                                                       0:size]
        '''if True:
    ...
    def alt_costfunction(self, newumatflat):
        ...
        OneRDM_loc = self.helper.construct1RDM_loc(self.doSCF,
                                                   newumatsquare_loc)
    ...
    def costfunction_derivative(self, newumatflat):
        ...
        for counter in range(len(newumatflat)):
            thegradient[counter] = 2 * np.sum(
                np.multiply(error_derivs[:, counter], errors))
        return thegradient
    ...
    def rdm_differences(self, newumatflat):
        ...
        for count in range(len(self.imp_size)):
            if (self.doDET == True
                ):  # Do density embedding theory: fit only impurity
                thesize += self.imp_size[count]
                assert (self.fitImpBath == False)
            else:  # Do density MATRIX embedding theory
                if (self.fitImpBath == True):
                    thesize += self.dmetOrbs[count].shape[1] * self.dmetOrbs[
                        count].shape[1]
                else:
        ...
        for count in range(len(
                self.imp_size)):  # self.imp_size has length 1 if self.TransInv
            if (self.fitImpBath == True):
                mf_1RDM = np.dot(np.dot(self.dmetOrbs[count].T, OneRDM_loc),
                                 self.dmetOrbs[count])
                ed_1RDM = self.imp_1RDM[count]
            else:
                mf_1RDM = (OneRDM_loc[:,
                                      np.flatnonzero(self.impClust[count])]
                           )[np.flatnonzero(self.impClust[count]), :]
                ed_1RDM = self.imp_1RDM[count][:self.imp_size[count], :self.
                                               imp_size[count]]
            if (self.doDET == True):  # Do density embedding theory
                if (self.doDET_NO == True):  # Work in the NO basis
                    theerror = np.diag(
                        np.dot(np.dot(self.NOvecs[count].T, mf_1RDM),
                               self.NOvecs[count])) - self.NOdiag[count]
                ...
                errors[jump:jump + squaresize] = np.reshape(theerror,
                                                            squaresize,
                                                            order='F')
                jump += squaresize
        assert (jump == thesize)
    ...
    def rdm_differences_bis(self, newumatflat):
        ...
        for count in range(len(self.imp_size)):
            # thesize += self.imp_size[ count ] * self.imp_size[ count ]
            mask_t = self.mask[np.ix_(range(jump, jump + self.imp_size[count]),
                                      range(jump,
        ...
        for count in range(len(
                self.imp_size)):  # self.imp_size has length 1 if self.TransInv
            mf_1RDM = (OneRDM_loc[:, np.flatnonzero(self.impClust[count])]
                       )[np.flatnonzero(self.impClust[count]), :]
            ed_1RDM = self.imp_1RDM[count][:self.imp_size[count], :self.
                                           imp_size[count]]
            theerror = mf_1RDM - ed_1RDM
            ...
            mask_t = self.mask[np.ix_(
                range(jumpc, jumpc + self.imp_size[count]),
                range(jumpc, jumpc + self.imp_size[count]))]
            squaresize = np.count_nonzero(mask_t)
            errors[jump:jump + squaresize] = np.reshape(theerror[mask_t],
                                                        squaresize,
                                                        order='F')
            jump += squaresize
    ...
    def rdm_differences_derivative(self, newumatflat):
        ...
        newumatsquare_loc = self.flat2square(newumatflat)
        RDMderivs_rot = self.helper.construct1RDM_response(
            self.doSCF, newumatsquare_loc, self.NOrotation)
        ...
        for countgr in range(len(newumatflat)):
            ...
            for count in range(
                    len(self.imp_size
                        )):  # self.imp_size has length 1 if self.TransInv
                if (self.fitImpBath == True):
                    local_derivative = np.dot(
                        np.dot(self.dmetOrbs[count].T,
                               RDMderivs_rot[countgr, :, :]),
                        self.dmetOrbs[count])
                else:
                    if (self.doDET == True) and (self.doDET_NO == True):
                ...
                if (self.doDET == True):  # Do density embedding theory
                    ...
                    error_deriv[jump:jump +
                                len(local_derivative)] = local_derivative
                    jump += len(local_derivative)
                else:  # Do density MATRIX embedding theory
                    squaresize = local_derivative.shape[
                        0] * local_derivative.shape[1]
                    error_deriv[jump:jump + squaresize] = np.reshape(
                        local_derivative, squaresize, order='F')
                    jump += squaresize
            assert (jump == thesize)
    ...
    def verify_gradient(self, umatflat):
        ...
        print("   Norm( gradient difference ) =",
              np.linalg.norm(gradient - gradientbis))
    ...
    def flat2square(self, umatflat):
        ...
        if (self.TransInv == True):
            size = self.imp_size[0]
            for it in range(1, self.Norb // size):
                umatsquare[it * size:(it + 1) * size,
                           it * size:(it + 1) * size] = umatsquare[0:size,
                                                                   0:size]
        '''if True:
        ...
        if (self.NOrotation != None):
            umatsquare = np.dot(np.dot(self.NOrotation, umatsquare),
                                self.NOrotation.T)
        return umatsquare
    ...
    def square2flat(self, umatsquare):
        ...
        if (self.NOrotation != None):
            umatsquare_bis = np.dot(np.dot(self.NOrotation.T, umatsquare_bis),
                                    self.NOrotation)
        umatflat = umatsquare_bis[self.mask]
        return umatflat

    def numeleccostfunction(self, chempot_imp):  #最小化使得电子数目相等
    ...
    def doselfconsistent(self):
        ...
        while (u_diff > convergence_threshold):
            ...
            rdm_diff = np.linalg.norm(
                rdm_old - self.transform_ed_1rdm()
            )  # self.transform_ed_1rdm()产生了一个块对角的矩阵，把所有的self.imp_1RDM拼接了起来
    ...
    def print_umat(self):
        ...
        squarejumper = 0
        for localsize in self.imp_size:  # self.imp_size has length 1 if self.TransInv
            print(self.umat[squarejumper:squarejumper + localsize,
    ...
    def transform_ed_1rdm(self):
        ...
        squarejumper = 0
        for count in range(len(
                self.imp_1RDM)):  # self.imp_size has length 1 if self.TransInv
            localsize = self.imp_size[count]
            result[squarejumper:squarejumper + localsize,
                   squarejumper:squarejumper +
                   localsize] = self.imp_1RDM[count][:localsize, :localsize]
            squarejumper += localsize
        return result
